...
export function ServiceCard({ service, categoryName }: { service: Service; categoryName: string }) {
  ...
  return (
    <div className="service-card-poca group flex min-w-0 flex-col overflow-hidden rounded-3xl bg-white shadow-soft md:rounded-[2rem]">
      <div className="relative h-48 w-full shrink-0 overflow-hidden md:h-56">
        ...
        <span className="absolute top-4 left-4 rounded-full bg-white/90 px-3 py-1 text-[10px] font-semibold uppercase tracking-wide backdrop-blur text-ink md:text-xs">
          {categoryName}
        </span>
      </div>
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col p-5 md:p-6">
        ...
        <div className="mt-2 mb-5 flex items-center gap-2 text-xs text-ink-light md:mb-6 md:text-sm">
          <Clock size={16} className="shrink-0 text-brand-500" style={{ color: "var(--color-primary)" }} />
          {formatDuration(service.duration_minutes)}
        </div>

        <div className="mt-auto flex items-center justify-between border-t border-brand-50 pt-4">
          <span
            className="text-lg font-semibold tabular-nums text-brand-600 md:text-xl"
