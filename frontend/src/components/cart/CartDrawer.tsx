...
export function CartDrawer({ open, onClose, onCheckout }: Props) {
  ...
  return (
    <>
      ...
      <div
        className="fixed top-0 right-0 z-[60] flex h-[100dvh] max-h-[100dvh] w-full max-w-md flex-col bg-surface shadow-2xl transition-transform duration-500 ease-in-out"
        style={{
        ...
        <div className="flex flex-1 flex-col gap-3 overflow-y-auto p-4 md:gap-4 md:p-6">
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <div
                key={item.service.id}
                className="flex flex-col rounded-xl border border-brand-50 bg-surface p-3 shadow-sm md:p-4"
              >
        ...
        {items.length > 0 && (
          <div className="border-t border-brand-100 bg-base p-4 pb-[max(1rem,env(safe-area-inset-bottom))] md:p-6">
            ...
            <button
              ...
              className="btn-primary-soft w-full rounded-xl bg-brand-500 py-3.5 text-center text-sm font-medium tracking-wide text-white shadow-lg shadow-brand-500/20 transition-colors hover:bg-brand-600 active:scale-[0.98] md:py-4"
            >
              Оформить запись
            </button>
          </div>
