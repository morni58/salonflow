...
export function Header({ tenant, onOpenCart }: Props) {
  ...
  return (
    <>
      ...
      >
        <div className="relative mx-auto max-w-[var(--layout-max)] px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between md:h-20">
            ...
            >
              {NAV_LINKS.map((link) => (
                <button
                  ...
                  className="text-xs font-medium uppercase tracking-wider text-ink-light transition-colors hover:text-brand-500"
            ...
            >
              <span className="hidden text-xs font-medium uppercase tracking-wider md:block">Ваша запись</span>
      ...
      <div
        className={`fixed inset-y-0 left-0 z-[110] flex w-4/5 max-w-sm flex-col bg-white shadow-2xl transition-transform duration-300 ease-in-out md:hidden ${
          drawerOpen ? "translate-x-0" : "-translate-x-full"
        }`}
        aria-hidden={!drawerOpen}
      >
        <div
          className="flex items-center justify-between border-b border-brand-100 px-6 py-5"
        ...
        <div className="border-t border-brand-100 p-6" style={{ background: "var(--color-bg)" }}>
          <button
            type="button"
            onClick={() => {
              setDrawerOpen(false);
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl py-4 text-sm font-medium tracking-wide text-white shadow-lg transition-colors"
            style={{
